...
Based on rjstone's discord-webhook-notify action.
...
- Use `beyonk-adventures/discord-webhook-notify@v1` if you want bugfixes but nothing backward-compatability breaking. (If anything breaks compatability it will be in v2+.)
- Use `beyonk-adventures/discord-webhook-notify@v1.0.0` (or another specific tag after the @) if you want to guarantee use of a specific revision.
...
```
- name: Test Success
    uses: beyonk-adventures/discord-webhook-notify@v1
    if: success()
    with:
        severity: info
